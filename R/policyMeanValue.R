...
#' @description  When the time index is an integer multiple of 200, this policy sets the current prices and supplies to the average of the previous 199 periods.
...
#' @seealso \code{\link{sdm2}}, \code{\link{gemDualLinearProgramming}}.
...
policyMeanValue <- function(time, state, state.history) {
  ...
  if (time >= span && (time %% span == 0)) {
    ...
    state$S <- apply(state.history$S[, , (time - span + 1):(time - 1)], c(1, 2), mean)
  }
