...
#' weighted.mean(x, w) - 0.5 * gamma * sd.p(x, w)^theta.
#' @describeIn AMSD
#' Computes the utility function mean(x) - 0.5 * gamma * sd.p(x)^theta or
#' weighted.mean(x, w) - 0.5 * gamma * sd.p(x, w)^theta. When theta == 2, it is the additive mean-variance utility function (i.e.
#' the function AMV).
...
#' @param w a numeric vector of weights (or probability).
...
#'
#' fun <- function(gamma.agt1 = 0.1, gamma.agt2 = 0.1,
#'                 S0Exg = matrix(c(
#'                   0.5, 0.5,
#'                   1, 1
#'                 ), 2, 2, TRUE),
#'                 dsav = 0.3) { # dsav is the demand structure adjustment velocity.
#'   sdm2(
#'     A = function(state) {
#'       Payoff <- USP %*% (state$last.A %*% dg(state$last.z))
#'
#'       VMU <- marginal_utility(Payoff, USP, list(
#'         function(x) AMSD(x, gamma = gamma.agt1, prob),
#'         function(x) AMSD(x, gamma = gamma.agt2, prob)
#'       ), state$p)
#'       Ratio <- sweep(VMU, 2, colMeans(VMU), "/")
#'
#'       A <- state$last.A * ratio_adjust(Ratio, coef = dsav, method = "linear")
#'       prop.table(A, 2)
#'     },
#'     B = matrix(0, 2, 2),
#'     S0Exg = S0Exg,
...
#'
#' ge <- fun(
#'   gamma.agt1 = 0.05, gamma.agt2 = 0.02,
#'   dsav = 0.03
#' )
#' matplot(ge$ts.p, type = "l")
#' }
#'
AMSD <- function(x, gamma = 1, w = rep(1, length(x)), theta = 1) {
  weighted.mean(x, w) - 0.5 * gamma * sd.p(x, w)^theta
}
...
#' weighted.mean(x, w) - 0.5 * gamma * var.p(x, w).
AMV <- function(x, gamma = 1, w = rep(1, length(x))) {
  weighted.mean(x, w) - 0.5 * gamma * var.p(x, w)
}
