...
#' @param sigma a scalar, which is the coefficient of elasticity of substitution between the
#' inputs. 1/(1-sigma) is the elasticity of substitution.
...
#' @examples
#' x <- 75
#' gemInputOutputTable_CES_3_3(input = matrix(c(
#'   200, 300, 100,
#'   x, 320, 530,
#'   250, 380, 0
#' ), 3, 3, TRUE), output = c(600, 1000, 630), sigma = -1)
...
gemInputOutputTable_CES_3_3 <- function(input = matrix(c(
  ...
  ge <- sdm(
    A = function(state){
      SCES_A(sigma=rep(sigma,3), alpha=alpha, Beta=Beta, p=state$p)
    },
    B = diag(3),
    S0Exg = {
      tmp <- matrix(NA, 3, 3)
